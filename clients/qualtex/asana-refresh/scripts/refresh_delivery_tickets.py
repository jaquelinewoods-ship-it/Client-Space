...
Fetches QGOMD delivery ticket statuses from Jira and writes a 'Delivery Tickets'
block into each matched Asana requirement task, updating it on every run.
...
The requirement-to-delivery-ticket mapping is loaded from:
  mappings/delivery_ticket_mapping.qualtex.json
"""
...
DEFAULT_CONFIG = REPO_ROOT / "mappings" / "delivery_ticket_mapping.qualtex.json"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "run-output"

# Match "Jira: QGOMR-54" in task notes (written by migrate.py)
JIRA_KEY_RE = re.compile(r"Jira:\s*(QGOMR-\d+)", re.IGNORECASE)
DELIVERY_MARKER_RE = re.compile(r"<h2>\s*Delivery Tickets\s*</h2>", re.IGNORECASE)
LAST_UPDATED_CLOSE_RE = re.compile(
    r"(?:<p>\s*)?<em>\s*Last updated:.*?</em>(?:\s*</p>)?", re.IGNORECASE | re.DOTALL
)
...
def fetch_asana_tasks(section_gid: str, headers: dict[str, str]) -> list[dict[str, Any]]:
    params = urllib.parse.urlencode({"limit": "100", "opt_fields": "gid,name,notes,html_notes"})
...
def parse_requirement_key(task: dict[str, Any]) -> str | None:
    """Extract QGOMR-XX from task notes (set by migrate.py)."""
    notes = task.get("notes") or ""
    match = JIRA_KEY_RE.search(notes)
...
def strip_existing_delivery_block(html_notes: str) -> tuple[str, bool]:
    ...
    if not last_updated:
        raise RefreshError(
            "Found Delivery Tickets marker but could not find Last updated paragraph; refusing to edit description"
        )
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        missing = [
            name
            for name, value in (("JIRA_EMAIL", jira_email), ("JIRA_API_TOKEN", jira_token), ("ASANA_PAT", asana_pat))
            if not value
        ]
        if missing:
        ...
        mapping = {str(k): list(v) for k, v in config["delivery_ticket_mapping"].items()}
        ...
        print(f"Fetching {sum(len(v) for v in mapping.values())} unique delivery tickets from Jira...")
        ...
        print(f"Fetching Asana tasks from section {section_gid}...")
        ...
        for task in tasks:
            ...
            requirement_key = parse_requirement_key(task)

            if not requirement_key:
                report["tasks_skipped"].append({
                    "task_gid": task_gid,
                    "name": task.get("name"),
                    "reason": "No QGOMR key found in task notes",
                })
                continue
            ...
            if not ticket_keys:
                report["tasks_skipped"].append({
                    "key": requirement_key,
                    "task_gid": task_gid,
                    "reason": "No delivery ticket mapping — add to delivery_ticket_mapping.qualtex.json",
                })
                continue

            missing_ticket_data = [k for k in ticket_keys if k not in delivery_data]
            if missing_ticket_data:
                report["tasks_skipped"].append({
                    "key": requirement_key,
                    "task_gid": task_gid,
                    "reason": "One or more Jira delivery tickets failed to fetch; not overwriting",
            ...
            try:
                block = build_delivery_block(
                    [delivery_data[k] for k in ticket_keys], report["timestamp_utc"]
                )
                ...
                base_before_strip, _ = (
                    strip_existing_delivery_block(html_notes)
                    if DELIVERY_MARKER_RE.search(html_notes)
                    else (html_notes, False)
                )
                ...
                if not preserved:
                    raise RefreshError("Preservation check failed; refusing to write task")

                if not args.dry_run:
                    update_asana_task(task_gid, updated_html, a_headers)
                    time.sleep(0.25)

                print(f"  {'DRY RUN' if args.dry_run else 'UPDATED'}: {requirement_key} — {len(ticket_keys)} ticket(s)")
                ...
                print(f"  FAILED: {requirement_key} — {exc}")
    ...
    print(f"\n{'='*60}")
    print(f"Updated : {len(report['tasks_updated'])}")
    print(f"Skipped : {len(report['tasks_skipped'])}")
    print(f"Failed  : {len(report['failures'])}")
    if report["jira_fetch_failures"]:
        print(f"Jira fetch failures: {list(report['jira_fetch_failures'].keys())}")
